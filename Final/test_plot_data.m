...
end_time = size(test_population_coordinates,1);

number_of_agents = size(test_population_coordinates,2)/2;
...
% NOTE: some lines have been made longer than represented in the test_population_coordinates
...
axis([0 max_x+1 0 max_y+1]);
  ...
  for a = 1:number_of_agents
      % draw person
      plot(test_population_coordinates(i,(2*a)-1),test_population_coordinates(i,(2*a)),'Color',[0 0 1],'Marker','.','MarkerSize',10);
      plot(estimated_position(i,(2*a)-1),estimated_position(i,(2*a)),'Color',[1 0 0],'Marker','.','MarkerSize',10);
  end
  pause(0.001);
end
